...
public class BlockServiceImpl extends ServiceImpl<BlockMapper, Block> implements BlockService {
    ...
    public Page<Block> getBlocksByPage(int pageNum, int pageSize, String sort) {
        ...
        // 验证排序字段
        if (!List.of("block_number", "reward", "timestamp", "transactions_count").contains(orderBy)) {
          // TODO 待对应，原接口是报404
        ...
        switch (orderBy) {
            case "block_number":
                if (isAsc) queryWrapper.orderByAsc(Block::getBlock_number);
                else queryWrapper.orderByDesc(Block::getBlock_number);
                break;
            case "reward":
                if (isAsc) queryWrapper.orderByAsc(Block::getReward);
                else queryWrapper.orderByDesc(Block::getReward);
                break;
            case "timestamp":
                if (isAsc) queryWrapper.orderByAsc(Block::getTimestamp);
                else queryWrapper.orderByDesc(Block::getTimestamp);
                break;
            case "transactions_count":
                if (isAsc) queryWrapper.orderByAsc(Block::getTransactions_count);
                else queryWrapper.orderByDesc(Block::getTransactions_count);
                break;
        }
