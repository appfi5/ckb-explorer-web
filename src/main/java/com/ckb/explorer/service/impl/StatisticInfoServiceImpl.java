...
public class StatisticInfoServiceImpl extends ServiceImpl<StatisticInfoMapper, StatisticInfo> implements StatisticInfoService {
  ...
  @Value("${statistic.averageBlockTimeInterval}")
  private int averageBlockTimeInterval = 100;
  ...
  public double hashRate(Long tipBlockNumber){
    ...
    for (Block block : blocks) {
      if (block.getDifficulty() != null) {
        totalDifficulties = totalDifficulties.add( BigDecimal.valueOf(
            Numeric.toBigInt(block.getDifficulty()).longValue()));
      }
    }
    ...
    BigDecimal hashRateValue = totalDifficulties.divide(new BigDecimal(totalTime), 6, BigDecimal.ROUND_HALF_UP);

    return hashRateValue.doubleValue();
  }
  ...
  public Long getTransactionsLast24hrs(Long timestamp) {
    ...
    // 使用MyBatis-Plus的sum方法直接计算24小时内的交易总数
    LambdaQueryWrapper<Block> queryWrapper = new LambdaQueryWrapper<>();
    queryWrapper.select(Block::getTransactionsCount)
        .ge(Block::getTimestamp, twentyFourHoursAgo)
        .le(Block::getTimestamp, timestamp);

    // 使用MyBatis-Plus的selectObjs方法获取所有交易数，然后用stream求和
    List<Object> transactionsCounts = blockMapper.selectObjs(queryWrapper);

    // 使用stream API计算总交易数
    return transactionsCounts.stream()
