...
public class CkbTransactionCacheFacadeImpl implements ICkbTransactionCacheFacade {
    ...
    @Override
    public Page<TransactionPageResponse> getTransactionsByPage(Integer page, Integer size, String sort) {
        // 设置默认排序
     ...
     * 从数据库加载交易列表
     */
    private Page<TransactionPageResponse> loadFromDatabase(Integer page, Integer pageSize, String sort) {

      // 执行分页查询
      Page<CkbTransaction> pageResult = ckbTransactionService.getCkbTransactionsByPage(page, pageSize, sort);
        // 转换为响应对象
        return CkbTransactionConvert.INSTANCE.toConvertPage(pageResult);
    }

    /**
     * 从数据库加载交易详情（这是一个示例方法，实际项目中需要替换为真实的数据库查询逻辑）
     * @param txHash 交易哈希
     * @return 交易详情响应
     */
