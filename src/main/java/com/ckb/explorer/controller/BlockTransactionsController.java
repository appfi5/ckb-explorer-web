...
/**
 * BlockTransactionsController 处理区块内交易的API请求
 * 对应Ruby代码中的Api::V1::BlockTransactionsController
 */
@RestController
...
public class BlockTransactionsController {
    ...
    public ResponseEntity<ResponseInfo> show(
        ...
        // 获取区块内的交易列表（通过缓存门面）
        var transactions = blockTransactionCacheFacade.getBlockTransactions(blockId,  req.getTxHash(), req.getAddressHash(), req.getPage(), req.getPageSize());
    ...
    private void validateQueryParams(String blockHash, String txHash, String addressHash) {
        ...
        // 验证地址哈希（如果提供）
        if (addressHash != null && !addressHash.isEmpty() && !queryKeyUtils.isValidAddress(addressHash)) {
